<template>
  <div class="app-container">
    ...
    <div :hidden="showMediaSet" id="app" class="container">
      <el-table height="100vh" v-loading="listLoading" :data="userBindMachineS" element-loading-text="Loading" fit
    ...
    <div :hidden="!showMediaSet">
      <div class="tab-responsive">
        <div v-for="setsitem,setsindex in usermediasets">
          <el-card size='mini' shadow="hover" :body-style="{ padding: '0px'}">
            <el-time-picker @change="playtimeChange" v-model="setsitem.playTime" value-format="timestamp"
              :picker-options="{
                  selectableRange: '00:00:00 - 23:59:59'
                }"
              placeholder="播放时间">
            </el-time-picker>
            <el-dropdown trigger="click" @command="handleCommand" style="margin:0px 10px;">
              <el-button type="text" :style="{'color':(setsitem.moviePath==''?'red':'#0DBADC')}">
                {{setsitem.movieName}}<i class="el-icon-arrow-down el-icon--right"></i>
              </el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="(mediaitem,mediaindex) in usermedias" :command="dropdownCommand(setsindex,mediaindex,setsitem,mediaitem)">{{mediaitem.moviename}}</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      ClickReturn() {
        let changed = JSON.stringify(this.usermediasets) != JSON.stringify(this.usermediasetslog);
        if (changed) {
